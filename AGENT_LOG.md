...
- **关键决策：** 自实现主循环（不寄生 LangChain/AutoGen 等）；机制即代码（护栏/反馈信号=自写校验器，非提示词）；LLM 抽象层可注入 mock；DeepSeek（OpenAI 兼容）；TypeScript 5.x + Docker。

### 2026-07-10 — writing-plans → PLAN.md

- **技能：** superpowers:writing-plans
- **事件：** 产出 `PLAN.md` 实现计划（`8279f99`），拆为 14 个 task，每个 task 含文件/接口/失败测试代码/绿灯门。
...
> 执行模式：superpowers:subagent-driven-development —— 每 task 一个 fresh implementer subagent（sonnet=glm-5.2，Task 12（Dockerfile）起改用 haiku=deepseek-v4-pro）+ 两阶段评审（spec 合规 + 代码质量）。每 task 开 `task/N` 分支 → push → 用户手动开 PR + merge → controller 更新 PLAN/ledger → 下一 task。
...
- **事件：** 实现 `ba6698c` on task/10，merge `63b989c` PR #8。run/config 两命令 + dotenv + 主入口守卫（`process.argv[1]?.includes('index')` 防止 vitest import 时副作用解析 argv）。绿灯门 49/49 + tsc 0。
...
### Task 12：Dockerfile（2026-07-12）

- **事件：** 主 agent 实现 `f591c58` + fix `1db3ac2` on task/13，merge `21dfc5c` PR #11。多阶段 Dockerfile（node:20-alpine：builder npm ci+build / 运行 npm ci --production+拷 dist，ENTRYPOINT node dist/index.js）+ .dockerignore（.env 不进镜像=安全，node_modules/.git/.superpowers 不进 context=提效）。
...
### Task 13：GitHub Actions CI（2026-07-12）
...
- **CI 首次实际运行 pass（§9.6 闭环）：** `09bea69` push 触发 CI，run 结论 success。这验证了：Dockerfile 在 CI ubuntu docker 上 build 成功（闭环 Task 12 "本地无 docker"）；npm test 在 Linux runner 上全过（harness.test.ts shell 在 Linux 正常，无 Windows cmd 噪音）。

### Task 14：AGENT_LOG.md 初始化（2026-07-12）
...
| 07-12 | Task 12（Dockerfile） | 补 COPY tsconfig.json | 评审 Critical C1，tsup dts 依赖 |
| 07-12 | Task 13（CI） | docker-build 加 needs: unit-test | Minor#1，测试失败不应构建镜像 |
...
2. **TDD + 两阶段评审有效捕获缺陷**——Task 5 的 regex 覆盖不足（Important）、Task 12（Dockerfile）的 tsconfig.json 拷贝缺失（Critical）都被评审挡下并修复，未流入下游 task。

3. **brief 笔误风险**——controller 写 brief 时字段名/签名须核对真实源码（Task 10 `tracerDir`→`tracesDir`）。implementer 不应顺手"修正"spec 而应反馈，由 controller 确认。

4. **冷启动验证的价值**——只有没看过实现的 agent 才能暴露 spec 的隐含假设；修订后的 SPEC/PLAN 在 14 个 task 实现中几乎没有因 spec 歧义返工。

5. **本地成功 ≠ 容器成功**——本地 build 有 tsconfig.json，Docker builder 阶段没拷则失败。Dockerfile 必须显式 COPY 构建所需全部文件。

6. **机制即代码的可验证性**——所有核心机制（护栏、反馈闭环、凭据加密）有 mock-LLM 确定性单元测试，移除真实 LLM 后仍能验证（机制演示①②③）。这是与"提示词工程"的根本区别。

7. **Minor 不阻塞但需 triage**——累积的 Minor（Task 8 死 import、Task 7 getTrace 浅拷贝、Task 6 脏数据守卫等）记入 ledger，留 final whole-branch review 集中 triage，避免 roll-up 无人读导致静默丢弃。
